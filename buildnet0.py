...
def load_data(train_file = "nn0.txt", test_file = "nn0.txt"):
    train_set = []
    test_set = []
...
def neural_network(nn_weights, string):
    ...
    output_weights = nn_weights.weights[INPUT_SIZE * HIDDEN_LAYER_SIZE:INPUT_SIZE * HIDDEN_LAYER_SIZE + HIDDEN_LAYER_SIZE * OUTPUT_SIZE]
...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        train_set, test_set = load_data("nn0.txt")

    best_individual, best_score = genetic_algorithm(train_set, test_set)
    write_output_to_file("wnet0.txt", best_individual)
